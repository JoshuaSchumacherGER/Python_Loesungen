...
def play_again():
    ...
    for point in XO_points:
        ...
        point.reset()
    status_label.configure(text="X's turn", bg='green')
    ...
    def set(self):
        ...
        if not self.value:
            ...
            if current_chr == "X":
                ...
                self.button.configure(bg='snow', fg='green')
                current_chr = "O"
                status_label.configure(text="O's turn", bg='red')
            elif current_chr == "O":
                O_points.append(self)
                self.button.configure(bg='snow', fg='red')
                current_chr = "X"
                status_label.configure(text="X's turn", bg='green')
        check_win()
...
def check_win():
    for possibility in winning_possibilities:
        if possibility.check('X'):
            status_label.configure(text="X won!", bg='orange')
            disable_game()
            return
        elif possibility.check('O'):
            status_label.configure(text="O won!", bg='orange')
            disable_game()
            return
    if len(X_points) + len(O_points) == 9:
        status_label.configure(text="Draw!", bg='blue')
        disable_game()
